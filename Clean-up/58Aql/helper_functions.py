...
def align_3_stacked(light_dir: str = "./", save_dir: str = "./", silent: bool = False) -> None:
    ...
    for i in range(0, len(files)):
        ...
        source_image = image_data[0].data + 0  # Here, the addition of zero (0) solves the the endian compiler issue.
        header       = image_data[0].header
        image_aligned, footprint = aa.register(source_image, reference_image, detection_sigma = 2)
        aligned_file = files[i].replace('.fits', '')
...
def CreateStarsTable(data: np.ndarray, iterative: bool = False, filter: str = 'Red', exp_time: float = 60,   \
    ...
    if(fwhm > 20.0):
        if(not silent): print(f'Warning: FWHM > 20.0. Fit went wrong: using \'fwhm\' = 10.0.')
        fwhm = 10.0
    ...
    if(plot_stars_flag == True):
        im_plot(data, title = f'Image with {len(phot)} stars found ({filter} filter)', cbar_label = 'Counts', cbar_flag = True, stars_found = phot, cmap = 'viridis', lo = 0.5, up = 99.9)
    ...
    phot['mag_err'] = np.sqrt( (2.5 * 1/phot['flux_fit'] * 1/np.log(10) * phot['flux_err'])**2 + \
                               (-k * 0.01)**2 + (-sec_z * 0.01)**2 ) # This only works if not 2DGaussian.
    # Also, we consider that the exposure time has uncertainty = 0.0s.
